...
function VideoModal({ initial, onSubmit, onClose }: VideoModalProps) {
  ...
  return (
    <form onSubmit={computeSubmit}
      ...
      {error && (
        <div 
        // style={{ backgroundColor: "#ffe5e5", border: "1px solid red", borderRadius: "6px", padding: "10px", marginBottom: "10px" }}
        >
          {error}
      ...
      > 
        Autoplay video? :<input 
        // style={{ display: "flex", alignItems: "center", gap: "8px", width: "20px", height:"20px"}} 
        type="checkbox" checked={autoplay} onChange={e => setAutoplay(e.target.checked)} /><br />
      </div>
      Size of TextBox:<div 
      // style={{ display: "flex", gap: "10px" }}
      >
